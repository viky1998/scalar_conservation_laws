...
class GodunovForIntersections:
    def __init__(self, f, f_prime, f_prime_inverse, rho_0, f_1, f_2, a, b, fineness, intersection: Intersection12) -> None:
        self.f = f
        ...
        self.cell_averages_in = self.get_initial_cell_averages()
        self.cell_averages_out_1 = self.get_initial_cell_averages()
        self.cell_averages_out_2 = self.get_initial_cell_averages()
        self.current_time = 0
    ...
    def calc_step_out_1(self, time_step):
        ...
        next_averages[-1] = last_averages[-1] + \
            (time_step/self.grid_size)*(self.f(self.riemann_solution(last_averages[-2], last_averages[-1])))-\
                (1/self.grid_size)*integrate.quad(self.f_2, self.current_time, self.current_time+time_step)[0]
        for ii in range(1,len(last_averages)-1):
            next_averages[ii] = last_averages[ii] + \
                (time_step/self.grid_size)*\
                    (self.f(self.riemann_solution(last_averages[ii-1], last_averages[ii]))-\
