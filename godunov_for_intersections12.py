...
class GodunovForIntersections:
    def __init__(self, f, f_prime, f_prime_inverse, rho_0_in, rho_0_out_1, rho_0_out_2, f_1, f_2_out_1, f_2_out_2, a, b, fineness, intersection: Intersection12) -> None:
        self.f = f
        self.f_prime = f_prime
        self.f_prime_inverse = f_prime_inverse
        self.rho_0_in = rho_0_in
        self.rho_0_out_1 = rho_0_out_1
        self.rho_0_out_2 = rho_0_out_2
        self.f_1 = f_1
        self.f_2_out_1 = f_2_out_1
        self.f_2_out_2 = f_2_out_2
        self.a = a
        self.b = b
        ...
        self.cell_averages_in = self.get_initial_cell_averages('in')
        self.cell_averages_out_1 = self.get_initial_cell_averages('out_1')
        self.cell_averages_out_2 = self.get_initial_cell_averages('out_2')
        self.current_time = 0

        self.f_rho_in_1 = self.intersection.f_rho_in_1
        self.f_rho_out_1 = self.intersection.f_rho_out_1
        self.f_rho_out_2 = self.intersection.f_rho_out_2
    ...
    def get_cfl_condition(self):
        cfl_in = self.grid_size/max(abs(self.f_prime(self.cell_averages_in[-1,:])))
        cfl_out_1 = self.grid_size/max(abs(self.f_prime(self.cell_averages_out_1[-1,:])))
        cfl_out_2 = self.grid_size/max(abs(self.f_prime(self.cell_averages_out_2[-1,:])))
        min_cfl = min(cfl_in, cfl_out_1, cfl_out_2)
        return min_cfl
    
    def numeric_iteration(self):
        cfl_temp = self.get_cfl_condition()
        self.cell_averages_in = np.vstack([self.cell_averages_in, self.calc_step_in(cfl_temp)])
        self.cell_averages_out_1 = np.vstack([self.cell_averages_out_1, self.calc_step_out_1(cfl_temp)])
        self.cell_averages_out_2 = np.vstack([self.cell_averages_out_2, self.calc_step_out_2(cfl_temp)])

        rho_in_1 = self.cell_averages_in[-1,-1]
        rho_out_1 = self.cell_averages_out_1[-1,-1]
        rho_out_2 = self.cell_averages_out_2[-1,-1]
        self.intersection.update(rho_in_1, rho_out_1, rho_out_2)

        self.current_time += cfl_temp

    def calc_step_in(self, time_step):
        next_averages = np.zeros(self.fineness)
        last_averages = self.cell_averages_in[-1,:]
        next_averages[0] = last_averages[0] + \
            (1/self.grid_size)*(integrate.quad(self.f_1, self.current_time, self.current_time+time_step)[0])-\
    ...
    def calc_step_out_2(self, time_step):
        next_averages = np.zeros(self.fineness)
        last_averages = self.cell_averages_out_2[-1,:]
        next_averages[0] = last_averages[0] + \
            (time_step/self.grid_size)*(self.f_rho_out_2-\
                self.f(self.riemann_solution(last_averages[0], last_averages[1])))
        next_averages[-1] = last_averages[-1] + \
            (time_step/self.grid_size)*(self.f(self.riemann_solution(last_averages[-2], last_averages[-1])))-\
                (1/self.grid_size)*integrate.quad(self.f_2_out_2, self.current_time, self.current_time+time_step)[0]
        for ii in range(1,len(last_averages)-1):
            next_averages[ii] = last_averages[ii] + \
                (time_step/self.grid_size)*\
                    (self.f(self.riemann_solution(last_averages[ii-1], last_averages[ii]))-\
    ...
    def iterate(self, times: list[int]):
        ...
        eval_rho_in = np.zeros((len(times), self.fineness))
        eval_rho_out_1 = np.zeros((len(times), self.fineness))
        eval_rho_out_2 = np.zeros((len(times), self.fineness))

        for i in time_indices:
            eval_time = times[i]
            while eval_time > self.current_time + self.get_cfl_condition():
                self.numeric_iteration()
            eval_rho_in[i] = self.calc_step_in(eval_time-self.current_time)
            eval_rho_out_1[i] = self.calc_step_out_1(eval_time-self.current_time)
            eval_rho_out_2[i] = self.calc_step_out_2(eval_time-self.current_time)

        return eval_rho_in, eval_rho_out_1, eval_rho_out_2